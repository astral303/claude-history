...
impl App {
    ...
    /// The spinner glyph for a search still running `SEARCH_SPINNER_DELAY`
    /// after its keystroke, so the renderer marks the previous query's count
    /// stale without reading the clock itself. A search that resolves sooner
    /// shows nothing, so a fast keystroke does not flicker.
    pub fn search_spinner_glyph(&self) -> Option<char> {
        ...
        Some(SEARCH_SPINNER_GLYPHS[frame % SEARCH_SPINNER_GLYPHS.len()])
    }
